...
{  public function read_vend()
    ...
    $this->db->select('id_proveedor, nombre_proveedor, rif_proveedor, direccion, telefono');
    $this->db->from('public.proveedor ');
   // $this->db->where('desincorporar !=', '0');
    ...
    public function listar_info($data){
        $this->db->select('id_proveedor, nombre_proveedor, rif_proveedor, direccion, telefono');
        $this->db->where('id_proveedor', $data['matricular']);
       // $this->db->where('desincorporar !=', '0');        
        // $this->db->join('vendedor v', 'v.id_vendedor = c.id_vendedor', 'left');    
        $query = $this->db->get('public.proveedor');
